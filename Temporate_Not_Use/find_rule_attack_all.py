...
if __name__ == "__main__":
    ...
    for file in list_files(folder_path):
        ...
        players_loc = ['E', 'S', 'W', 'N']
        turn_counts = {loc: 0 for loc in players_loc}
        meld_counts = {loc: 0 for loc in players_loc}
        ...
        for j in range(1, len(states.state)):
            ...
            if action == 'M':
                turn_counts[actor_loc] += 1
            elif action in ('P', 'E', 'EM', 'EL', 'ER', 'UG', 'HG', 'G'):
                meld_counts[actor_loc] += 1
            
            if action in ('HD', 'MD'):
                player_state = states.get_player(states.state[j-1], actor_loc)
                ...
                is_winner = 1 if actor_loc == winner_loc else 0
                
                remaining_turns = (winner_total_turns - turn_counts[actor_loc]) if is_winner == 1 else None
                
                snapshot = {
                    '檔案名稱': file.name,
                    '玩家位置': actor_loc,
                    'Step_ID': j,
                    '當下巡數': turn_counts[actor_loc],
                    '當下副露數': meld_counts[actor_loc],
                    '向聽數 (Shanten)': shanten,
                    '進張數 (Ukeire)': ukeire,
                    '剩餘牌山': states.state[j].mountainCount,
                    'Target_是否胡牌(Win)': is_winner,
